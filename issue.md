...
* **Current State** (2025-06-26 - RESOLVED):
  ...
  * ❌ **OAuth Authentication Broken**: External auth URL `https://auth.openhands.redacted` now returns SSL handshake failure.
...
| 2025-06-26 10:50 | **NEW ISSUE**: OAuth authentication fails with SSL handshake error | ❌ `https://auth.openhands.redacted` broken |
...
* [x] **Cloudflare Bypass**: External `openhands.redacted` works despite Gateway TLS issue
* [x] **Auth Domain Blocked**: Cloudflare cannot bypass Gateway TLS issue for `auth.openhands.redacted`
...
* ✅ HTTPRoute: Configured for `auth.openhands.redacted` → keycloak service
...
* ✅ <https://llm-proxy.redacted/> does work properly
...
1. ✅ **Domain Simplification**: Changed from `auth.openhands.redacted` → `auth.redacted`
...
- ❌ **App Redirect Mismatch**: OpenHands app still redirects to `auth.openhands.redacted` instead of `auth.redacted`
- ❌ **Keycloak Internal URLs**: Keycloak redirects to `https://keycloak/` instead of external `https://auth.redacted`
